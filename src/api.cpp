...
static_assert(sizeof(float) == 4 && std::numeric_limits<float>::is_iec559,
              "F32 requires IEEE 754 binary32");

extern "C" {
int ac_sample_bytes(int format) {
  switch (format) {
    case AC_U8: return 1;
    case AC_S16: return 2;
    case AC_S24: return 3;
    case AC_S32: case AC_F32: return 4;
    default: return 0;
  }
}
...
ac_convert_fn ac_get_converter(int src, int dst, int64_t target) {
  if (!ac_sample_bytes(src) || !ac_sample_bytes(dst) || src == dst) return nullptr;
  if (target == AC_TARGET_NATIVE) target = ac_choose_target(AC_TARGET_NATIVE);
  else if (target != AC_TARGET_C &&
           (target < 0 || (target & (target - 1)) != 0 ||
            (ac_supported_targets() & target) == 0)) return nullptr;
