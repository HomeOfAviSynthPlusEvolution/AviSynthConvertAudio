...
int main() {
  ...
  for (int src = AC_U8; src <= AC_F32; ++src)
    for (int dst = AC_U8; dst <= AC_F32; ++dst) {
      ...
      if (std::memcmp(expected.data(), output.data(), count * ac_sample_bytes(dst)))
        return 2;
      ...
      for (int round = 0; round < 7; ++round)
        for (int step = 0; step < 2; ++step) {
          ...
          for (int i = 0; i < 100; ++i)
            functions[k](input.data(), output.data(), count);
          auto start = std::chrono::steady_clock::now();
          for (int i = 0; i < repeats; ++i)
            functions[k](input.data(), output.data(), count);
          times[k][round] =
              std::chrono::duration<double, std::nano>(std::chrono::steady_clock::now() - start).count() / repeats;
          checksum += output[count * ac_sample_bytes(dst) / 4 - 1];
        }
      for (auto& t : times)
        std::sort(t.begin(), t.end());
      std::printf("%s,%s,%d,%.3f,%.3f,%.3f\n", names[src], names[dst], count, times[0][3], times[1][3],
                  times[1][3] / times[0][3]);
    }
